...
const ResumeMakerForm: React.FC = () => {
  ...
  const handleCheckATS = () => {
    navigate('/smart-score', { state: { importedResume: formData } });
  };
  ...
  const handleGenerateAI = async (id: string, type: 'work' | 'project' | 'summary', title: string, context: string, currentText: string, isRegenerate: boolean = false) => {
    ...
    // 1. IRON-CLAD SECURITY: Block anonymous users instantly
    const token = localStorage.getItem('token');
    if (!token || token === 'undefined' || token === 'null') {
      alert("Please Log In or Register to unlock premium AI writing features!");
      navigate('/login');
      return; 
    }
    ...
    try {
      ...
      // 2. DEFENSIVE PARSING: Prevent crashes if the server sends HTML instead of JSON
      ...
      if (data.success) {
        ...
        // Handle expired/invalid tokens securely
    ...
    <div className="max-w-[1600px] mx-auto p-4 lg:p-8 mt-4 mb-10 text-gray-800">
      ...
      <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          {/* The New ATS Button Location */}
          <button 
            type="button" 
            onClick={handleCheckATS} 
            className="bg-purple-600 hover:bg-purple-700 text-white font-bold text-sm px-4 py-2 rounded-xl transition-all shadow-sm flex items-center gap-1.5"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 12h4l2-9 5 18 3-9h6"/></svg>
            Check ATS Score
          </button>

          {/* The Existing Credits Badge */}
